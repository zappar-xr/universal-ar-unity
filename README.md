...
The ZapWorks CLI or server does not send a `Content-Encoding` header for Unity's gzip/br files, so it's best to enable `Publishing Settings > Decompression Fallback` in Unity2020 or above, or disable `WebAssembly Streaming` in Unity 2019.
